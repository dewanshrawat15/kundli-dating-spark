...
-- First, delete existing profiles that were created with default values
DELETE FROM public.profiles 
WHERE name = 'User' 
  AND date_of_birth = '1990-01-01' 
  AND place_of_birth = 'Unknown'
  AND time_of_birth = '12:00:00';

-- Insert Indian girl profiles with proper data for users who don't have profiles
...
SELECT 
  au.id,
  au.email,
  -- Use row_number() with modulo to ensure different names for each user
  CASE ((row_number() OVER (ORDER BY au.id)) % 21)
    ...
    ELSE 'Anjali Kumar'
  END as name,
  -- Random birth date between 1996-2002 (ages 22-28) - fixed type casting
  ('1996-01-01'::date + INTERVAL '1 day' * ((row_number() OVER (ORDER BY au.id)) % 2190)) as date_of_birth,
  CASE ((row_number() OVER (ORDER BY au.id)) % 10)
  ...
  -- Different times using row_number - fixed type casting
  (TIME '06:00:00' + INTERVAL '1 hour' * ((row_number() OVER (ORDER BY au.id)) % 17)) as time_of_birth,
  'straight'::sexual_orientation as sexual_orientation,
  CASE ((row_number() OVER (ORDER BY au.id)) % 3)
  ...
  CASE ((row_number() OVER (ORDER BY au.id)) % 15)
