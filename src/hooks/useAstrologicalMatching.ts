...
export const useAstrologicalMatching = () => {
  ...
  const validateProfileData = (profile: any, profileName: string) => {
    ...
    if (!profile.name || typeof profile.name !== "string") {
      throw new Error(
        `${profileName} name is missing or invalid: ${JSON.stringify(profile)}`
      );
    }
  ...
  const recordInteraction = async (
    targetUserId: string,
    interactionType: "viewed" | "liked" | "passed"
  ) => {
    ...
    try {
      console.log(
        `Recording ${interactionType} interaction for user ${targetUserId}`
      );
      ...
      console.log(
        `Successfully recorded ${interactionType} interaction:`,
        data
      );
      ...
      if (interactionType === "liked" && currentProfile) {
        ...
        if (
          ...
          (!existingMatch.match_score ||
            !existingMatch.compatibility_description)
        ) {
          ...
          if (updateError) {
            console.error(
              "Error updating match with compatibility data:",
              updateError
            );
          } else {
          ...
          const { error } = await supabase.from("profiles_match").insert({
            user_a_id: orderedUserA,
            user_b_id: orderedUserB,
            status: "active",
            match_score: currentProfile.compatibilityScore,
            compatibility_description: currentProfile.compatibilityDescription,
          });
          if (error) {
            console.error("Error adding profiles_match data", error);
          }
        }
      }
  ...
  const fetchAndRankProfiles = useCallback(
    async (isRefetch = false): Promise<void> => {
      ...
      if (
        ...
        setError(
          "Complete your astrological profile (birth date, time, and place) to see matches"
        );
        return;
      }
      ...
      const fetchPromise = (async () => {
        try {
          ...
          const { data: unseenProfiles, error: fetchError } =
            (await supabase.rpc("get_unseen_profiles", {
              requesting_user_id: user.id,
              city_filter: profile.current_city,
              limit_count: 10,
            })) as { data: ProfileData[] | null; error: any };
          ...
          if (fetchId !== lastFetchIdRef.current || !mountedRef.current) {
            console.log(
              `⚠️ Fetch #${fetchId} cancelled (newer request or unmounted)`
            );
            return;
          }
          ...
          console.log(
            `✅ Fetch #${fetchId} got ${unseenProfiles?.length || 0} profiles`
          );
          ...
          console.log(
            `🧮 Processing ${unseenProfiles.length} profiles for compatibility in fetch #${fetchId}`
          );
          ...
          try {
            ...
            console.error(
              `❌ Profile validation failed in fetch #${fetchId}:`,
              validationError
            );
          ...
          const validProfiles = unseenProfiles.filter((unseenProfile) => {
            if (
              !unseenProfile.date_of_birth ||
              !unseenProfile.time_of_birth ||
              !unseenProfile.place_of_birth
            ) {
              console.log(
                `⚠️ Skipping profile ${unseenProfile.name} in fetch #${fetchId} - incomplete birth data`
              );
              return false;
            }
            ...
            try {
              validateProfileData(
                unseenProfile,
                `Target profile ${unseenProfile.name}`
              );
              return true;
            } catch (validationError) {
              console.error(
                `❌ Validation failed for ${unseenProfile.name} in fetch #${fetchId}:`,
                validationError
              );
              return false;
            }
          ...
          if (validProfiles.length === 0) {
            console.log(
              `📭 No valid profiles with complete birth data in fetch #${fetchId}`
            );
            if (isRefetch && mountedRef.current) {
              setProfiles([]);
            }
          ...
          if (fetchId !== lastFetchIdRef.current || !mountedRef.current) {
            console.log(
              `⚠️ Fetch #${fetchId} cancelled before compatibility call`
            );
            return;
          }
          ...
          console.log(
            `🔮 Calling astrological compatibility for ${validProfiles.length} profiles`
          );
          ...
          console.log(
            `🔮 Making compatibility request for fetch #${fetchId} with ${validProfiles.length} profiles`
          );

          // Make batch compatibility request
          const { data: batchCompatibilityData, error: compatibilityError } =
            await supabase.functions.invoke("astrological-compatibility", {
              body: {
          ...
          if (fetchId !== lastFetchIdRef.current || !mountedRef.current) {
            console.log(
              `⚠️ Fetch #${fetchId} cancelled after compatibility call`
            );
            return;
          }
          ...
          if (compatibilityError) {
            console.error(
              `❌ Compatibility error in fetch #${fetchId}:`,
              compatibilityError
            );
          ...
          const batchResult =
            batchCompatibilityData as BatchCompatibilityResponse;
          console.log(
            `✅ Compatibility results for fetch #${fetchId}:`,
            batchResult
          );

          // Create ranked profiles with compatibility data
          const rankedProfiles: RankedProfile[] = validProfiles.map(
            (unseenProfile) => {
          ...
          console.log(
            `📊 Ranked ${rankedProfiles.length} profiles for fetch #${fetchId}`
          );
  ...
  const handleLike = useCallback(async () => {
    ...
    try {
      ...
      if (
        ...
        console.log(
          `🔄 Only ${remainingProfiles} profiles left, fetching more...`
        );
